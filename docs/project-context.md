...
**Baseline as of 2026-09-04.** `uv run pytest` (**no `-q`** — `pyproject.toml` already sets it, and a second one suppresses the `N passed` line **and the `collected N items` header**, so the collected figure needs `--collect-only`) = **2886 passed** in ~333 s, on a tree carrying this machine's gitignored spec artifacts; a clean checkout collects fewer, because `test_spec_frontmatter.py` parameterises one test per spec file on disk. The figure before this read ~~1916 passed as of 2026-08-24~~. `uv run ruff check .` clean. `node --check` clean on both `web/assets/app.js` and `web/assets/api.js`.
