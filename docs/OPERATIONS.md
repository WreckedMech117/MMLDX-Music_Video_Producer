...
> `e2e_shot_controls.py`, were failing from before Epic 10 until 2026-08-31 — ~~are failing
> today~~ — and **they failed for two different reasons, one of them a product defect**
> (Epic 11 retrospective item 86; see `deferred-work.md` for the closing note).
>
> * `e2e_seed_and_asset_tabs.py` was right and the application was wrong. A submission route
>   writes its job record **before** it submits, so a Render again that ComfyUI refuses has
>   still moved the project's revision — and that revision is the token every shot save
>   carries. The browser kept showing the number the Director typed while `PUT /shots`
>   answered 409 for the rest of the session. `resyncAfterRefusedSubmission` in `app.js` now
>   re-reads the project on the failure path exactly as the success path already did, on all
>   five single-submission buttons. Pinned by two executed contract tests.
> * `e2e_shot_controls.py` was stale and the application was right to have changed — except
>   about its own wording. Render again gained a `confirm()` on 2026-08-19 and the script
>   never learned to answer it; it now answers **Cancel**, which keeps its no-GPU contract.
>   Two assertions in it had also been superseded: a toast takes clicks again since
>   2026-08-19 (see the toast bullet below), and a `clear_toasts` call sat one line after the
>   hit test that needed it. The dialog itself was **wrong**: it promised `turbo` for eight
>   days after the click stopped asking for a bundle, and now names the project's own.
...
> things about all ~~twenty-four~~ ~~twenty-five~~ ~~twenty-six~~ twenty-seven, none of which needs a browser: **each one parses, each one
...
uv run --with selenium python tests/e2e_overlap_band.py          # default port 8785
uv run --with selenium python tests/e2e_transition_preview.py     # default port 8786
...
uv run --with selenium python tests/e2e_planning_mode.py   # default port 8791
```

These twenty-seven **start and prove their own server** and take no base URL — `--port N` overrides. Order does not matter and they share no state; each creates a fresh temporary data root under `%TEMP%\mvp-<label>-<nonce>`, left behind as evidence.
...
`e2e_overlap_band.py` is Epic 11's gate: the Overlap band on the SHOTS track and the two transition rows in the Effects tab. It is the slice this repository has been wrong on most often — eleven defects across Epics 9 and 10 passed every automated gate and were caught only by a screenshot — so it asserts the things only a browser answers. **It reads the paint back**: a typed band's fill resolved to `--blue` at 22% (the stylesheet declares an 8-digit hex before the `color-mix`, so a browser that resolves neither would draw a band with no fill at all, which is the *measured thing that draws as nothing* class), an untyped one's `--line-strong` hatch with no `--blue` anywhere in it, and both edges at a real 1px. It measures the band against the clips it is about, checks the clip text underneath is still painted, and — the reason it exists beside `e2e_clip_overlap_and_split.py` rather than inside it — **hit-tests every right resize handle lying under a band**, because R-40 put the band *above* the clips and an overlay at the handles' z-index would take back the 2026-08-21 fix. It measures every label the served catalogue can produce in the band's own font and asserts `api.js`'s per-character budget bounds them, so the threshold that decides whether a band letters is a measurement rather than a guess. Then it drives the gestures: setting `Transition out` and reading the mirror's toast and both stored fields, a pair-only type refused with the route's own sentence and nothing written, and a real drag on the resize handle that takes the Overlap away — after which the band is gone, the stored types are retained, and the change is announced. **Section 9 is story 11.f7's**: a Shot nested wholly inside its neighbour is a boundary the export refuses, and the timeline used to draw it as an ordinary blue blend with a `paired` row. It asserts the refused band is a `--line-strong` cross-hatch in a box closed on all four sides with **no `--blue` and no `--red`** anywhere in it (the palette judgement is in `DESIGN.md` §3), that it is distinguishable from the untyped band beside it, that the row states all three frame counts and offers no preview control, and it measures the note for clipping at 1600 px and 820 px. It also reads the band's own edge row pixel by pixel, because the correction that pass made — a dashed edge read as one noisy stripe against the clip border drawn *through* the half-transparent hatch one pixel inside it — is invisible to every computed-style assertion and was found only by opening the picture. **Ten screenshots go to `test-artifacts/`** *(corrected 2026-08-31; this said ~~Six~~ and went stale in the commit that added sections 8 and 9)*. It queues nothing, contacts a dead ComfyUI port, and never reaches `/prompt`.

`e2e_transition_preview.py` is story 11.5's gate and needs **ffmpeg on PATH**: it is the blend a Director can look at before exporting. The route is covered offline with real ffmpeg and the row's states are covered in the contract tests; what only a browser answers is whether the control is reachable, whether the sentence that replaces it is drawn, and whether the clip that comes back **decodes and plays**. A `<video>` whose `src` 404s is indistinguishable in the DOM from one that is playing, so it waits on `readyState` and `videoWidth` and compares them against the width and height the route said it served. It drives the real gestures: an Overlap with nothing chosen saying which absence it is, a type chosen through the select, the blend watched, a second type served as a **different** file, and the incoming Shot's `Transition in` row offering the identical clip because a boundary is addressed by its outgoing Shot (AD-30). Its last section is the defect this story found by looking rather than by reading: a Shot with a one-sided transition and **no effect stack at all** is now previewed, where before `shotPreviewWanted` refused to ask and the Monitor showed the untreated take while the export shipped the fade. Six screenshots go to `test-artifacts/`. It queues nothing, contacts a dead ComfyUI port, and never reaches `/prompt`.
...
`e2e_shot_controls.py` seeds five shots and four assets through shipped routes and drives mark-ready/mark-draft, render-again and the multiview promote control — asserting each is rendered, hit-testable at its centre, correctly labelled, and that a disabled button the browser honours changes nothing server-side. It never clicks promote, and asserts `jobs` is empty at the end. Render again puts a `confirm()` up and this script answers **Cancel** — the arm that re-opens the shot and queues nothing, which is what keeps `jobs` empty — asserting first that the question names the bundle the project would actually render on. The accept arm is driven instead by `e2e_seed_and_asset_tabs.py`, which owns a dead ComfyUI port for it. `e2e_song_context.py` drives the lyrics/style editor, its counters, save, the per-field restores, the clearing confirmation through the browser's real dialog, and the VRAM eject toggle — writing `machine-preferences.json` inside its own root, so your stored choice is untouched. It causes exactly one deliberate 422 (the oversized sheet), declared to the console gate by name. `e2e_document_clearing.py` is that clearing confirmation's counterpart for the three creative documents (R-18): it seeds all three through `PUT /documents`, then drives Edge's real dialog for an emptied Treatment (whose sentence must *not* name a Restore), an emptied Brief (whose must, and the button it names is then clicked and the text brought back), a retyped document and an already-empty one (neither may ask), and two documents cleared in one save (one dialog, one sentence per side of the partition). It queues nothing and never reaches `/prompt`.

`e2e_suggest_video_pass.py` is story 13.2's browser half: the indicator a Director watches while Suggest Video runs, and the control that stops watching it. **It runs against a recording double, never a live model** — `MVP_LLM_BASE_URL` is pointed at a stub this script owns, which answers `/v1/chat/completions` after a delay it is told to take and with the reply it is told to give (complete, thin, or prose carrying no tool call, two of which make the route's real 502). That delay is the whole reason the indicator has an observable life: a real pass takes minutes and cannot be made to fail on demand. It asserts the **paint** rather than the DOM — the dot's resolved `--amber`, its measured 9×9 box and `50%` radius, and its resolved `animation-name` both with `prefers-reduced-motion` emulated through CDP and without, because the still dot is the opt-out and a `no-preference` rule is the only form that also serves a browser which never answers the query. It reads the elapsed reading as the browser renders it and watches it climb, checks the indicator carries no percentage, `<progress>` or `role="progressbar"`, and asserts that when the reply lands the ticking number is gone from the element that held it — the two elapsed numbers are never on screen together. **Its last two sections are the ruling of 2026-09-04**: the abandon question is read off Edge's real dialog and dismissed once before it is accepted, then the double is allowed to finish, the page is reloaded, and the Brief holds what the pass wrote with **Restore armed** — which is the whole reason it is safe to say the pass cannot be called back. Eight screenshots go to `test-artifacts/`. It queues nothing, contacts a dead ComfyUI port, and never reaches `/prompt`.

`e2e_planning_mode.py` is story 14.2's gate: Planning Mode, the mode the browser holds (TP-6, AD-35). **It runs against a recording double, never a live model** — `MVP_LLM_BASE_URL` is pointed at a stub this script owns, which answers `/v1/chat/completions` with a `write_brief` tool call carrying whichever Brief it has been told to write. The safety property itself is not what this proves: a consent-less request refused after a consented one is asserted through the real route by `tests/test_planning.py`, and the enter/exit controls and the composer's routing are *driven* against a stub DOM by `tests/test_frontend_contract.py`. What only a browser answers is whether the **suspended control looks suspended** — `.lock-toggle.superseded` is a stylesheet rule, and a rule that loses on specificity leaves a checkbox that is `disabled` and otherwise looks exactly like the thing that decides, which is invisible to every offline gate here. So it reads the paint: the resolved `text-decoration-line` on the label, the superseded micro-label's measured box and resolved `--amber`, the bar's own box measured **above** the document tabs (DESIGN §4.1 puts it over the documents, not beside the chat), its dot at a real 9×9 resolved to `--amber`, and the bar genuinely gone at `display: none` and zero height with the mode off. It drives the two things only a real browser answers about the state — a **reload** leaves the mode off, and a **project change** takes it away with the tick handed back unticked — and it drives the locked-Brief edge case to the end: the bar says planning cannot write it, the request is sent anyway, and the sentence the Director reads back is `document_lock_refusal`'s, from the server. Seven screenshots go to `test-artifacts/`. It queues nothing, contacts a dead ComfyUI port, never reaches `/prompt`, and causes no console entry at all.

`e2e_seed_and_asset_tabs.py` gates the seed randomizer and the Assets subtabs, the two 2026-08-20 Director requests. It measures the shortened seed box and the toggle beside it at two window widths, drives the toggle through roll-once/hold/per-shot/hand-typed/re-roll-on-Render-again, and reads **every seed back from the server** rather than off the input. It points `MVP_COMFY_URL` at a dead port it chose, so the three Render agains it drives cannot reach the user's ComfyUI: each 502s, and the settled *never submitted* record is what the script waits on for the gesture to be over — the wait that is missing is how a typed seed lands inside a submission still in flight. It then walks all seven asset subtabs at three widths, measures that the clips library takes zero height on an asset tab and the grid reaches the bottom of the panel, and that a tab change loses neither the selected shot nor the selected asset. It writes three screenshots into `test-artifacts/` — `-seed-row`, `-clips-tab`, and the closing one — because the two things it was built for are shapes on a screen.
...
- **A toast covered the shot inspector's controls.** `.toast-region` is `pointer-events: none`, so the gaps between toasts pass clicks through to the control beneath. *Amended 2026-08-31: this said `.toast-region` **and `.toast`** are `pointer-events: none`, and the second half stopped being true on 2026-08-19.* `9bd9447` gave the toast itself its handler and its pointer events back, deliberately — an error toast has no timer at all, so a dead click-shield would trap stale text over the workspace — and the guarantee since is the weaker one: a toast takes exactly one click, to dismiss itself, and the control underneath is reachable on the next. The comment in `styles.css` said both things eight lines apart and has been corrected too. Relocating the region was rejected: every corner of this layout holds a control in some workspace — the two inspectors bottom-right, the import and Flux actions bottom-left, the waveform actions top-right, the rail top-left — so moving it trades one collision for another and only in the workspace you happened to check. A toast here carries nothing clickable and self-removes after 4.2 s, so it has no claim on a pixel. The visual overlap remains by choice; the Director can now act through it. `e2e_shot_controls.py` **presses `#compile-shot` through a live toast** and asserts the press arrived, with a helper that records the toast really was standing over the button so the assertion cannot go vacuous.
