...
> things about all twenty-four, none of which needs a browser: **each one parses, each one
...
These twenty-one **start and prove their own server** and take no base URL — `--port N` overrides. Order does not matter and they share no state; each creates a fresh temporary data root under `%TEMP%\mvp-<label>-<nonce>`, left behind as evidence.
