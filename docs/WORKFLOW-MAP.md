...
- **Sampling profiles (added 2026-08-18, neither rendered live from this application yet).** `H3Request.profile` selects one of two configurations, each reproducing a whole source of evidence rather than mixing them. `H3_REFERENCE_PROFILES` in `workflows.py` holds them as data.
  ...
  A third profile, **`turbo-references2v`**, reproduces the Director's canonical `MiniMaxH3Turbo References2V` mode: `minimax_h3_turbo_v4_step600_ema_pruned_comfyui.safetensors` at strength **1.0** with `simple`/8/`euler`, every value read out of `h3-turbo-references2v-user-export.json`. It is named after the source graph rather than after a speed claim. Two evidenced turbo bundles from two different Director graphs ship side by side, each whole — the shipped `turbo` came from the LTX music-video pipeline.

  **`turbo-references2v` has now been rendered — six takes, 2026-08-23**, which retires the "nothing has been rendered on it" line this section carried since 2026-08-18. Three shots from the Director's live project, each rendered twice at one seed on `default` and on `turbo-references2v`, reported in `docs/measurements/2026-08-23-bundle-comparison/index.html`. See "Which path picks which bundle" below for what it found.

  **Which path picks which bundle (2026-08-23).** One setting: `Project.sampling_profile`, written by `PUT /api/projects/{id}/sampling-profile` and by nothing else — the generic full-project `PUT` re-adopts the stored value in both directions, as it does for `default_setting_id`. `H3Request.profile` and `GenerateBatchRequest.profile` are `SamplingProfile | None`, and `None` (which every shipped client sends) resolves to the project's choice in `app.resolved_sampling_profile`. So **Generate All, Re-queue flagged and Render Again all render the same bundle**, and the browser shows which one, with its step count, before the batch is confirmed.

  Until that date the two paths silently disagreed: both `api.generateBatch` call sites sent no profile and fell through to `"default"` (20 steps), while `app.js`'s Render Again hardcoded `"turbo"` (4 steps). The same project rendered two different graphs depending on which button was pressed, and nothing on screen named either number. **The default is unchanged** — an untouched project renders the byte-identical payload, pinned by `H3_REFERENCE_PAYLOAD_DIGEST` in `tests/test_api.py`.

  **Which bundle produced which take (2026-08-23).** Because the setting is per-project and the Director changes it between renders, a project's takes are a mixture, so `generate_h3` records the bundle onto the job it creates: `RenderJob.sampling_bundle` carries the name *and* the `steps`/`sampler`/`scheduler`/`lora`/`lora_strength` it resolved to, from `workflows.resolved_h3_sampling` — the same call `build_h3_reference_payload` makes, so the record cannot describe a bundle other than the one submitted, including when `H3Request.steps` overrode the count. The values are what make a later retune of `H3_REFERENCE_PROFILES` *detectable*: compare a job's record against the table it names. The keyframe and text-only graphs record `NO_EVIDENCED_BUNDLE` rather than a blank, and a job written before the field carries nothing and reads as **unknown** — never as `default`, which would be a fabricated measurement. Recording it moved no payload: `H3_BUNDLE_PAYLOAD_DIGESTS` and `H3_BUNDLE_STEP_OVERRIDE_DIGESTS` pin all three bundles through both doors, plus a step override, against `cd5c785`.

  **Reference shots only, and it is enforced.** A bundle names a reference-graph configuration. A profile a caller *names* on a keyframe or text-only shot is still refused with a 422 (see below); a profile *inherited* from the project is not, and those graphs go on rendering their own 20-step bundle — because refusing an inherited one would drop every keyframe and text-only shot out of Generate All for a bundle their graphs were never asked to use.
  ...
  **`turbo-references2v` against `default`, measured 2026-08-23 at production length.** Three shots at 141/158 frames, same seed, same references, same song window; full report in `docs/measurements/2026-08-23-bundle-comparison/`. **Lip-sync is indistinguishable** — envelope correlation differs by 0.005 or less on every shot (0.8454/0.8404, 0.9876/0.9825, 0.9664/0.9638) and the lag is identical to 0.1 ms in all three pairs. **The picture is sharper, not softer**: measured high-frequency energy is higher on every shot (+127%, +152%, +52%), at the cost of waxier skin and flatter mid-tone micro-texture at high zoom. **No hands are clearly in frame in any of the six takes, so hands are untested.** On the one pair where both arms ran clean and back to back (158-frame b-roll) the saving is **2.0×** on ComfyUI execution — 479.1 s against 236.6 s — and it is *the step count*: per step the two bundles cost the same (20.6 s default against 22.8 s turbo), and fixed per-render overhead of ~65–75 s dilutes the 2.5× step ratio to about 2×. One arm is excluded from every ratio: `wide/default`'s 2929.5 s ran during a VRAM-pressure episode. The Director's ruling on the trade: **"both still look good so up to user"**, which is why the bundle is a control rather than a new default.

  **A fixed seed is bit-exact, and only while the model stays resident — measured 2026-08-23.** Four renders of one byte-identical payload at seed 14 produced two pairs: within a pair every one of the 141 frames is byte-for-byte identical and every §8 picture statistic agrees to the last digit; across the pairs the takes differ by mean |Δ| 16.7/255 at r=0.58, which is the same size as a *prompt change*. The variable is not time, not the session and not the seed — between the pairs ComfyUI evicted and reloaded the model **unprompted**, and its own `execution_cached` list correlates perfectly with the pixels. So the sampler is deterministic on this card and the nondeterminism lives in the load-and-encode path; which node in that path carries it is unresolved at n=2. Two consequences that reach code. **Any claim that a re-render at the same seed reproduces the take must carry the residency condition** — `RESUBMIT_SEED_STRIDE` in `api.js` states it, and every wording downstream of it. And **a byte-identical resubmission is not a render at all**: ComfyUI served one from its execution cache in 1.157 s with the sampler reported cached and the previous file re-saved under a new name, which is the load-bearing reason the resubmit seed stride exists. Reusing an archived take as an experimental control is unsafe for a third reason — a payload digest cannot tell you whether a reload happened in between. Full run: §8.8's follow-up in `_bmad-output/planning-artifacts/h3-attention-backend-experiment.md`.
  ...
  **All five attention profiles have now been rendered — measured 2026-08-21/22**, which retires the "nothing has been rendered on any non-default profile" line this section carried until 2026-08-23. All five were screened at 107 frames on the `default` bundle and produced timings: `default` 9.08, `comfy-kitchen` 9.18, `sage-auto` 9.22, `sage-fp8-cuda++` 9.37, `pytorch` 11.41 s/it — and `pytorch` was then promoted to 226 frames and **did not complete**, cancelled at 97 minutes having reached zero sampling steps, which is the measurement that identified the 158→226 cliff as a memory wall rather than a compute one. Full run, its three harness defects and what each voided: §6.4 and §6.9–6.13 of `_bmad-output/planning-artifacts/h3-attention-backend-experiment.md`. Three standing caveats on those numbers — they are `default`-bundle 20-step figures, the ranking is what reproduces across fixtures rather than any single absolute, and `sage-auto` doubles as the harness's own control.

  Each profile is also schema-audited by `tests/preflight_h3_ultra.py` against live `/object_info` — node registered, input names matching, combo value in the published list — and a schema audit is still not evidence of a frame. `tests/measure_h3_attention.py` is the A/B; it refuses to submit without `--confirm-gpu`, and it treats a run that ComfyUI silently fell back to PyTorch on as **inconclusive** rather than as "no difference", because `ModelAttentionBackend.VALIDATE_INPUTS` returns `True` for any string.
  ...
  **A cited video's soundtrack is deliberately not conditioned, and the payload now says so** (2026-08-26). `MiniMaxH3MediaLoader` can pair a video's own audio track with it, or route that track into the standalone `audios` group, and it does neither unless `media_state` carries `has_audio`. The submit route sent neither `has_audio` nor `audio_mode` until this date, so the track was discarded on every submission and nothing anywhere said it had been — reachable today, since `app.js` derives the `video` kind from an upload's MIME type and `models.citation_slot_kind` wires it through. **The decision is that it stays off**: a reference shot is performed against the *master song*, which `use_song_audio` attaches beside it and which the over-render window exists to line the take up against, so a clip's own soundtrack in the same sampling pass is unrelated music; turning it on by default would change what every existing citation renders; and nothing here can decide *how* — no Asset field records whether a video carries an audio stream, and no control offers the paired-versus-standalone choice the node reads. What changed is the silence: the route now writes `has_audio: False` explicitly, so the choice is on the wire rather than resting on the node's default, and `batch.video_soundtrack_note` reports it per shot in the readiness list, under its own heading ("Soundtrack not sent") in the muted colour that says "a cost, not a problem", with no badge on the clip because nothing is wrong with the shot. Turning it *on* is a Director's call and would need both of those missing pieces first.

  **The wiring is derived from one mirror of the loader's own partition** (2026-08-26), `workflows.partition_h3_references`. The splitter's outputs are per-group and positional, and three of the loader's rules had never been modelled on this side: a reference whose `enabled` is exactly `False` consumes no slot and the numbering closes up around it; `video_audios` is positional against `videos`, so a silent video still holds its place; and a video whose audio is `standalone` joins the `audios` group where the *video* sits in the list, displacing every `audio` reference after it. The condition inputs and the split output indices are now taken from that one partition, so a slot the loader leaves empty cannot be wired and a slot it fills cannot be missed. **The per-kind ceilings are the larger of two counts, because the node counts its own media twice and the two disagree**: `VALIDATE_INPUTS` counts every item of a kind including the switched-off ones and refuses a tenth picture or a fourth video before the graph runs, while `_partition` skips them and then truncates each group silently — and the audio group is not bounded by the attached count at all, since a `standalone` video's soundtrack joins it. Three attached audios plus one such video is four audios, and the fourth would be wired to split output 18, which the splitter does not have. Neither count alone catches both. It is a **mirror, not an import** — the extension is GPL, outside this project and not a dependency — and it is pinned by tests in `tests/test_workflows.py` that state the node's rules as their expectations. `audio_mode` is only ever compared against `"paired"` and `"standalone"`: any other spelling falls through both branches and sends no soundtrack, so a test using a third value proves nothing about either rule.
  ...
  **A profile *named* on the request, not one inherited from the project** (2026-08-23). The keyframe branch draws the same line in the same words. `request.profile is not None` is what separates them: naming a bundle a graph cannot apply is the silent mis-logging above, while `Project.sampling_profile` is a standing preference only the reference graph has evidence for — and refusing that would quietly drop every text-only and keyframe shot out of Generate All the moment the Director chose turbo. The control in the browser states the scope where the choice is made.
...
| H3 Ultra reference/audio shot | yes | explicit 18-node adapter, 19 on either turbo profile | yes | **live render verified end to end 2026-08-18** — one picture reference plus the master song, 90 frames at 3.750 s with synchronized audio; rendered at 4 steps on the **default** profile, so the path is proven and the picture is not usable, and character consistency is **not** demonstrated. All three sampling bundles have since been rendered from this application: **`turbo` on 2026-08-18** (same sheet, same shot, same seed, one variable changed) and **`turbo-references2v` on 2026-08-23** (six takes at production length, `docs/measurements/2026-08-23-bundle-comparison/`). The line that stood here until 2026-08-23 — turbo "schema-audited only… never been rendered from this application" — contradicted the 2026-08-18 verification recorded two sections up on the day it was written | yes |
