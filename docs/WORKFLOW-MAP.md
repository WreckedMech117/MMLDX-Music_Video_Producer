...
**Consequence worth knowing before you hit it:** the product must stay inside the encoder's 0.04–360 s range, so at the default headroom **a duration above 240 s is refused** (300 × 1.5 = 450). The 30–300 s range is unchanged, but the headroom now decides which durations are actually submittable, and 300 s requires a headroom of 1.2 or less.
