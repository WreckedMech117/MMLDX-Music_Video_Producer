...
- **Sampling profiles (added 2026-08-18, neither rendered live from this application yet).** `H3Request.profile` selects one of two configurations, each reproducing a whole source of evidence rather than mixing them. `H3_REFERENCE_PROFILES` in `workflows.py` holds them as data.
  ...
  On the turbo profile the adapter inserts `mvp:lora` between `mvp:model` and `mvp:shift`, so every node downstream of the loader draws from the LoRA. **Only the bundles are proven, and only in the Director's own ComfyUI graph — not through this application.** Neither the LoRA in isolation nor either scheduler paired with the other profile's sampler has been rendered by anyone here, which is why the profile is one choice rather than four switches.

  Two deliberate non-changes: the adapter keeps `shift_audio=3` from the audited export rather than the Director's `6`, and the graph keeps `PathchSageAttentionKJ` enabled where the Director's node is bypassed. Both are outside the sampling bundle the story adopted; neither has been compared live.
