...
| **Treatment Planning** | 12–17 | 17 | **Slices A and B shipped 2026-09-03, Slice E on 2026-09-04 and Slice E2 the same day** (Stories 12.1, 12.2 — the Brief's lock, slot, restore and contract; Story 14.1 — the three planning tools, the per-request consent gate and `POST .../planning/turn`; Story 13.1 and the headless half of 13.2 — `POST .../brief/suggest`, one retry, and the extracted `app.write_document`; ~~**13.2's in-progress indicator is not built** — it is browser work, slice E2~~ **13.2 is whole: E2 shipped the browser half** — the Suggest video control, the amber elapsed-time indicator, `prefers-reduced-motion` through the `no-preference` form, the abandon that stops watching and says so, and `tests/e2e_suggest_video_pass.py` against a recording double). Everything else planned, not built |
...
**Treatment `BUILD-ORDER.md`'s Slice C or F, or Epic 17 — the effects `BUILD-ORDER.md` has no unbuilt slice left, treatment Slices A and B shipped 2026-09-03 and Slices E and E2 on 2026-09-04.** *(This said ~~Slice C, E or F~~ until the commit that shipped Slice E, and ~~Slice B~~ before that — the fifth time a "where to start" line in this document has named work that was already in `git log`, and the second in a row where the line and the commit that falsified it are the same commit, which is why this one moves in that commit rather than a pass later. Slice D still wants A + B + C, so C is what unblocks it; F needs only A + B. ~~**E2 is Story 13.2's other half** — the amber pulsing dot, `prefers-reduced-motion`, staying usable and abandoning the pass — and it needs a browser harness, which is why Slice E deliberately shipped without it.~~ **E2 shipped 2026-09-04** and took the browser harness with it (`tests/e2e_suggest_video_pass.py`, port 8789, a recording double for the model). This line is being moved in the pass that falsified it rather than one pass later, which is the third time that has been the correction and the second in a row.)* The effects slices A (song analysis), B (the chain builder), C (the Effects tab), D (preview), E (reactive binding) and F (transitions) are all built. Pick from the independent quick wins below, or from §6's open items — which is where the effects work that is genuinely still owed now lives.
