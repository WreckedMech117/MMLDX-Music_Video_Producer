...
function renderTimeline() {
  ...
  //
  // The ranking is `api.songOrderRanks` rather than a sort written here, and that is the fix for
  // 2026-08-19's defect rather than a tidy-up: this line used to hold its own sort while
  ...
  for (const card of model.cards) {
    ...
    if (card.handle.draggable) {
      ...
      $("#effect-card-" + card.index, inspector)?.addEventListener("keydown", (event) => {
        ...
        // Which way the key moves a card is `effectNudgeDirection`'s answer, not this handler's,
        // for the reason the tab strip above defers to `shotTabAfterKey`: a keyboard contract
        // that lives inside a listener cannot be executed by anything but a browser.
        const direction = effectNudgeDirection(event.key);
        if (!direction) return;
...
async function saveProject(notice = "Project saved") {
  ...
  // The route writes every document from the body, so a save carrying an empty box deletes what
  // is stored -- and what that costs is not the same for all three. Only the Brief keeps the
  // version this save displaces; the Treatment's and the Style bible's slots hold whatever a
  // Director reply last replaced and this save does not touch them, which is R-18's ruling and
  // the reason the question's own sentence is derived from `documentCapturedOnSave` rather than
  // written per document here.
  //
  // Asked only for that: replacing stored text with nothing. Replacing it with different text is
  // typing, and a question on every save teaches the Director to click through this one.
  //
  // `false`, not a bare `return`: this function's answer means "did the save reach the server",
  // and the lock handler re-seeds its checkbox on `false`. A decline that answered `true` would
  // leave a lock on screen the server never took.
  const cleared = documentClearing(state.project, documents);
  if (cleared.length && !window.confirm(documentClearingQuestion(cleared))) return false;
  try {
...
async function restoreDocument(documentKey) {
  ...
  try {
    ...
    markDocumentsSaved();
    renderAll();
    // Whether this restore can be undone is decided the way the server decides it:
    // `reversible=bool(displaced.strip())`, where `displaced` is the text the swap pushed
    // into the slot -- so after the reply it is simply whether the slot now holds anything.
    // Derived from the returned project rather than added to the wire, because the reply is
    // the whole project and a second field carrying a fact already in it is a second
    // authority on it. Until 2026-09-03 this side had only the symmetric sentence, so the
    // toast promised a swap back while the thread said the restore was one-way and the
    // button beside them was greyed out.
    const kept = state.project?.[`${documentKey}_previous`];
    toast(documentRestoreNotice(documentKey, { reversible: Boolean(kept && kept.trim()) }));
  } catch (error) {
    toast(error.message, "error");
...
function syncBeatMarkersControl() {
  ...
  if (!button) return;
  // The sentences and the pressed state are `beatMarkersControlPlan`'s, not this function's --
  // `syncSnapTargetsControl` reads a plan for exactly this reason and the two toggles shipped in
  // one epic should not disagree about where a control's words live.
  const plan = beatMarkersControlPlan(beatMarkersOn);
  button.classList.toggle("snap-on", plan.on);
  button.setAttribute("aria-pressed", plan.pressed);
  ...
  shotSaveChain = shotSaveChain
    ...
    .then((saved) => {
      ...
      if (revision === shotSaveRevision) {
        // The Effect Stacks come back from the reply, because that route owns them: it re-adopts
        // an existing Shot's stored stack whatever this body said, and it mints a card id for
        // every card arriving on a *new* Shot. Without this a Split's two halves both claim the
        // source's card ids and the next slider drag on the new half is refused by name for
        // naming a card that Shot does not hold. `adoptedShotEffects` decides it; this writes it.
        if (state.project?.id === projectId) {
          state.project.shots = adoptedShotEffects(state.project.shots, saved?.shots);
        }
        state.shotsDirty = false;
        state.dirty = state.documentsDirty;
