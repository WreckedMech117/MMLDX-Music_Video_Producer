...
- Promote an approved character to the installed Krea 2 QuadView multiview workflow — run live from the application on 2026-08-18, producing a 1536×1024 four-view sheet.
...
`smoke_songplanner_app.py` refuses to submit without `--confirm-gpu`, generates two short songs (invented then known-lyrics), and **creates one project per adapter** — a shared project's second run would clobber the first, since every music job targets the same `"song"` slot. Its per-variant JSON on stdout is the only record of which adapter produced which prompt, so keep it. `smoke_h3_app.py` is gated the same way and runs the H3 pre-flight audit before submitting. `smoke_h3_reference_app.py` is gated the same way and spends **two** GPU jobs rather than one — a Krea multiview promotion and then the H3 reference render — so budget for both before starting it. Full procedure in `docs/OPERATIONS.md`.
...
Five routes are verified end to end from this application. The text-only H3 path: on 2026-08-16 a 3.75 second shot rendered through live ComfyUI 0.33.1 and returned a 90-frame video with synchronized audio. Both SongPlanner paths: on 2026-08-17 the invented-lyrics and known-lyrics adapters each produced a real song, measured by `ffprobe` at 29.989 seconds of 44.1 kHz stereo FLAC from a 30 second request — the model resolves its own length, and 30 seconds is the shortest song the workflow accepts. For both songs the ComfyUI `/view` URL the player uses returned decodable FLAC; a browser was not driven, so browser playback of a generated song is not claimed. Krea multiview promotion and the H3 Ultra reference render: on 2026-08-18 an uploaded character image was promoted to a 1536×1024 four-view sheet in 56.1 seconds, the child asset's path was populated by the ordinary job reconciliation, and that sheet — attached to a shot alongside the project master song — rendered in 136.5 seconds to a 640×384 h264 clip of exactly 90 frames at 3.750 seconds with synchronized 32 kHz stereo audio at 3.744 seconds. `latest_output` was written and `approved_output` stayed empty. Every other route is built and unit-tested but has not produced a real render from this application. Post-processing, final assembly, BPM/section analysis, and multi-take approval remain planned. Their controls are disabled or explicitly scoped rather than presented as complete.

The reference render deserves its own sentence, because "verified" here means the path works and nothing more. It was rendered at **4 steps**, the cheapest setting that could prove the path, and the picture is heavily degraded — expected at that setting, and not a defect, but it is not usable output and no claim of picture quality is made from it. One thing the run surfaced is unresolved: in the first frame the reference sheet's four-panel layout appears to have carried into the composition rather than acting purely as an identity reference. No cause is asserted; whether a production step count, `ref_image_size="max"`, or a single-view reference changes it is untested. So **character consistency — a character recognizably the same person across the shots of a finished video — is not demonstrated.** The mechanism that would carry it is.
