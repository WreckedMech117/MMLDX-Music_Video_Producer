...
- source_spec: `_bmad-output/implementation-artifacts/spec-8-1-analyze-the-song-into-an-envelope.md`
  ...
  resolved: Director ruling 2026-08-24 — and the framing was wrong, which is the part worth keeping. The reason does not need a field to carry it; it needs to be *derived*, exactly as validity is. `analysis_absence_reason` recomputes it at read time from `shutil.which("ffmpeg")` and an ffprobe of the file, so the endpoint distinguishes six causes without storing one byte, and the same manifest answers differently once ffmpeg is installed. `SongAnalysis` still has no reason field and no valid flag — asserted. The endpoint remains read-only: it computes no envelope and writes nothing, asserted by a test that stubs `analyze_song` to raise if it is ever called. The import-path WARNING logging is unchanged.
