...
**Notes:** The known blocker is that existing combined exports regenerate the Shot from creator-specific media instead of accepting an approved input; a standalone adapter is the prerequisite. The dimension-boundary failure (SeedVR2 emitting 1250×720 into an LTX VAE requiring a multiple of **32**) is diagnosed, patched, and now proven live, so it is not the risk — the adapter is. `[NOTE FOR PM]` Consider proving the adapter early as a spike, so viability is known while there is still time to react.

> **Amended 2026-08-17 — the constraint is 32, not four.** This note said the LTX VAE requires "a multiple of four". Four is only the VAE's patchify size; its *total* spatial compression is 32 (patchify 4 plus three stride-2 stages), and it sets `crop_input = False`, so nothing auto-corrects an odd size. Believing the constraint was four is what produced the original `divisible_by=16` patch, which cleared patchify but left height 720 (720/32 = 22.5). The live boundary run (prompt `a64a0460-64e6-4a14-b207-e644bf9bda5d`, `success` in 17 min 36 s) settled it: `ffprobe` measured the LTX stage at 2496×1408, exactly 2 × 1248×704 through the subgraph's 2× latent upsample, where 16 would have produced 2496×1440. The full chain is now proven end to end, but **that does not weaken the drop condition above** — the run used the combined reference graph that regenerates the Shot, which is the very blocker FR-23 exists to remove. Matching amendments in `ARCHITECTURE-SPINE.md` AD-12, `epics.md`, and the brief addendum.
