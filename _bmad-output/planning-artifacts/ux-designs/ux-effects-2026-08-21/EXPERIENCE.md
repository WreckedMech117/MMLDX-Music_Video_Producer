...
**And it is previewed on the Shot's own picture** *(added 2026-08-29 by story 11.5)*. A one-sided transition treats the outgoing Shot's last frames, so the Monitor's Preview Clip for that Shot carries the treatment — the seventh fingerprint slot, reserved for exactly this. **Including on a Shot with no Effect Stack at all**, which is the one place story 9.2's *"an empty stack is not previewed"* had to give way: with no effects and a fade-out, the Shot's picture and its take are different pictures, and the Monitor was showing the take and saying nothing. Found by looking at the Monitor rather than by reading the route, which had been rendering it correctly the whole time.
...
- ~~**A Transition previews in place**: when the playhead is inside or approaching an Overlap, the Monitor's window spans the boundary, so the outgoing Shot, the Transition, and the incoming Shot play as one continuous piece (FX-21).~~ **Amended 2026-08-29 by story 11.5. The clip is exactly what this sentence describes — one window spanning the boundary, both Shots and the blend as one continuous piece — and it plays in the transition row rather than in the Monitor.**

  **Why, and it is a scope judgement rather than a disagreement.** R-35 makes a boundary its own subject with its own route and its own fingerprint, because `preview_fingerprint` takes one take and one window and asserts as an invariant that a preview is *"never one half of a resolved overlap"*. The Monitor is built on the other side of that line: `monitorPreviews` is keyed by Shot, `monitorPreviewView` layers a clip onto a per-Shot view, and `previewTime` maps the playhead into *that Shot's* window. A boundary clip has a different time axis — it starts before the Overlap and ends after it — so folding it in means that one function carries two subjects and two mappings. And *"inside or approaching"* is an automatic request: a 0.50 s Overlap in an 8 s song would have the Monitor swapping source twice per pass, spending the render budget on pictures nobody asked to look at, where the measured cost of a boundary clip is 144.6 ms at the house delivery size.

  **What ships instead is the same picture at the moment it is being decided.** The paired transition row gains a `Watch blend` control and a full-width clip under it: press it and the outgoing Shot, the blend and the incoming Shot play, looping, with the twelve type names an arm's length away in the select above. A boundary with no blend gets a sentence saying *which* absence it is instead of the control.

  **The Monitor placement is a better product and is recorded as deferred rather than rejected.** Seeing the transition arrive in the picture you are already watching, at speed, in context, is what this bullet was reaching for. Taking it means giving `monitorPreviewView` a second subject, deciding when a boundary clip supersedes a Shot clip on the same two layers, and choosing a rule for *approaching* that does not re-render on every tick. None of that is hard; all of it is a slice.
