...
| Overlap with a Transition set the export **will not blend** | `--line-strong` cross-hatch inside a closed 1px `--line-strong` box, no `--blue` and **no `--red`**, Consolas `NO BLEND` label — the boundary hard-cuts, and the band must say the outcome rather than the type |
...
So the label sits along the band's **bottom** edge, on a translucent `--bg` ground.

**A third band state, and the palette judgement it needed.** *(Added 2026-08-31 by story 11.f7.)* `assembly._paired_transitions` refuses a boundary whose split would lay no frames of the outgoing Shot, no frames of blend, or a *backwards* incoming stretch — and the timeline knew nothing about it, so it drew the ordinary `--blue` fill and a `paired` row over every one of them. A Director set a Dissolve, saw a blend, and exported a hard cut.

The mock this slice was approved against said *"hatched + `--red` hairline"*. **That is overturned, and the reason is standing design law 7:** `--red` means *error*, and this is not one. The Director arranged these windows deliberately, the export succeeds, and the plan records a sentence rather than refusing the render. The outcome is a hard cut — which is exactly what the untyped band above already is — so painting one of the two hard cuts red while the other takes no accent would say the export had failed. `--amber` means *running* and was rejected for the same kind of reason. `.effect-band-note` is the precedent already in the stylesheet: *a binding that cannot resolve is a state of the song, not a fault in the stack, so it takes no --red and no --red-edge.*

So the state is carried by **texture and closure**, never by hue: a cross-hatch at half the untyped band's period, inside a box closed on all four sides where the untyped band has no side edges at all, with the label changed from the type's name to `NO BLEND` and the whole sentence — which type is set, that it will not blend, and where to read why — on `title` and the accessible name at every width (UX-DR15). The row carries the measurement: *"Will not blend — on the assembly grid this boundary is 120 frames of shot 04, a 360-frame blend, then -240 frames of shot 05, so the export cuts here."* It takes `--dim` like any inert row and a **dashed** left edge, because an unoverlapped row is `--dim` too and the edge must not be the only thing separating them.

**The band's own edge is solid, and it was dashed for one screenshot.** Dashed is `.shot-clip.no-prompt`'s idiom for a thing declared and absent, which is what this is. What only a screenshot shows: the clip's own border sits one pixel inside the band's and is already drawn through the half-transparent hatch as a dotted `--amber` or `--acid` line — R-40's *readable through* working as designed — so a dashed band edge put a second broken line immediately above a broken one and the pair read as one noisy multicoloured stripe. This is the fourth thing in this section found by looking at a picture rather than by a gate. A clip is 82 px and its prompt stops painting at 47.8 px (59.8 px with a RENDERING line), so the bottom of the band is the one strip where the clip draws nothing at all. The ground is not a shadow and not chrome (§5 bars both); it is the same device the existing `.job-status` and `.source-badge` chips already use.
