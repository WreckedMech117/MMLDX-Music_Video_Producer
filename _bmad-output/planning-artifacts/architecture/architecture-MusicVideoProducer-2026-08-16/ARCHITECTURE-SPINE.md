...
### AD-13 — Approval is `approved_output`, nothing else `[AMENDED 2026-08-18]`

> **Amendment.** The FR-21 implementation writes `approved_output` **and** `status: "approved"` together, through exactly one writer (the approve route), reversed only by un-approve, with `approved_output == latest_output` pinned as an invariant. AD-13's *goal* — two representations of approval must not drift — is enforced by a different mechanism than the one it prescribed: instead of killing the status literal (which the brownfield code already read in `RENDER_AGAIN_STATUSES` and the mark-ready refusals), the two representations are written and cleared as one, with a behavioral single-writer test plus a source scan. ~~The `approved_start`/`approved_duration` window snapshot below remains live and unbuilt~~ **Built with Assembly, 2026-08-18** (Story 5.2, as this amendment required): approve snapshots, un-approve clears, both inside the same single writes, and assembly refuses a moved window by shot ID — with one addition the rule below did not anticipate: `approved_duration == 0` means *never snapshotted* (`duration` is `gt=0`, so zero is unrepresentable), and a pre-snapshot approval refuses with distinct re-approve wording rather than the stale wording.
