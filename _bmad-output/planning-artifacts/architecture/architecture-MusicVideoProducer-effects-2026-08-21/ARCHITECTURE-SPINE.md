...
```mermaid
graph TD
  FE[web/assets ES modules] -->|REST only| APP["app.py + routes/ (8 modules)"]
  APP --> FX[effects.py]
  APP --> AUD[audio.py]
  APP --> ASM[assembly.py]
  APP --> TL[timeline.py]
...
and `store.py`, and they import request models, refusal sentences and the `_adopt_*` helpers back
out of `app.py`, which is why `create_app` imports *them* from inside its own body. Read
`routes/__init__.py`'s docstring before adding a route: it explains the `register(app)` pattern,
why `APIRouter`/`include_router` is unusable here (it stops `app.routes` being a flat list, and
three of this repo's guards walk it), and what holds each pinned route where it is.
...
- **Rule:** A Parameter Binding compiles to a `sendcmd` commands file — one timed line per analysis tick — generated by a pure function of `(Song Envelope, binding, Shot window)` and asserted in tests by string comparison, exactly as ffmpeg argv already is. It is written beside the render's other inputs and passed to ffmpeg as a **bare relative filename with the process's cwd set to that directory**: ~~an absolute Windows path's drive-letter colon parses as a filter option separator and fails with `No option name near 'frame'`, naming a filter that is not the problem. Reproduced 2026-08-21.~~ **The cause stated here was wrong, and is corrected 2026-08-27 after re-measuring on ffmpeg 7.0.** It said an absolute Windows path fails because *the drive-letter colon parses as a filter option separator*. It does not: `sendcmd=f=C:/dir/name.cmds`, forward slashes, plain and unquoted, **renders correctly** — drive-letter colon and all — and keeps working when the directory name holds a space. What actually breaks is a character the **filtergraph** splits on: a comma gives `No such filter: 'comma/t1.cmds'`, and a path holding `=` or `&` gives `No option name near '...'` — which is the message AD-22 quoted, so the original measurement was almost certainly taken on such a path and the cause misattributed to the drive letter. **And the two remedies are not actually different**: `lut3d`'s single-quoted colon-escaped form `'C\:/dir/name.cmds'` survives even a directory named `hard, dir; [x] =y & 100%` for `sendcmd` too. The cwd-relative remedy still works and is what ships (a generated script name is `[a-z0-9_.-]` only, needs no escaping, and keeps an absolute path out of the composed chain — which matters because the chain is now the preview cache key). It is a choice, not the only escape from a parser bug that does not exist. **The lesson is the one this project keeps relearning: a remedy can be right while the reason given for it is false, and the false reason is what the next reader generalises from.** **A second correction, 2026-08-27:** *"one timed line per analysis tick"* is one line per tick **per filter option**. Temperature, Lift, Gain and Banding Suppression each spend one Director-facing dial across two, three and four options, so those emit two to four lines at every timestamp. `eval=frame` expressions are **not** a second supported mechanism; if introduced later they must produce the identical visible result or not exist (FX-NFR-3).
...
> generation, and transition-segment argv."* It sits beside two amendments that *are* descriptive of
> as-built code, so a reader has every reason to take it the same way, and an Epic 10 implementer
> would open the module expecting to extend a `sendcmd` writer that is not there.
>
> | Clause | Status |
> |---|---|
...
> shipped, so a plain sentence beside one is read as shipped too. Say which.
...
| ffmpeg file arguments | ~~**Never a bare absolute Windows path** — the filtergraph parser splits at the drive-letter colon and blames the wrong filter.~~ *(cause corrected 2026-08-27 — the drive-letter colon is fine; a comma, `=` or `&` anywhere in the path is not. See AD-22.)* Beyond that the remedy is per-filter, measured, not one rule: `lut3d`'s `file=` takes the **single-quoted, colon-escaped** form `'C\:/x/y.cube'`, and `sendcmd`'s `f=` takes a **cwd-relative** name with the process cwd set to its directory (AD-22). See below before choosing |
...
(AD-22) — ~~the drive-letter colon there fails with `No option name near 'frame'`, naming a
filter that is not the problem~~ *(cause corrected 2026-08-27: the drive-letter colon does not fail; a
filtergraph-splitting character in the path does, and the quoted form fixes both filters. See AD-22.)*.
...
src/music_video_producer/
  ...
  routes/        # THE ROUTE LAYER since 2026-08-26 — 8 modules, 60 of 76 routes.
                 #   shots.py   <- GET/PUT .../shots/{id}/effects  (and Epic 10's bindings)
                 #   song.py    <- POST .../song/analyze, GET .../song/envelope
                 #   timeline.py<- GET .../timeline/snap-targets
                 #   project.py <- PUT /api/projects/{id}  (replace_project)
                 #   unsorted.py<- GET /api/effects/catalogue
                 #   assets.py, render.py, context.py, __init__.py (read its docstring)
  app.py         # the 16 pinned routes — incl. POST .../shots/{id}/preview and .../assemble —
                 #   plus request models, refusal sentences and every _adopt_* helper, which
                 #   the route modules import back out of here. + _adopt_shot_effects
