...
FX-18: One-sided transitions treat the Shot's own final ~~or opening~~ frames, followed ~~or preceded~~ by a hard cut *(amended 2026-08-30: **only the outgoing half is built.** `_compose_one_sided_transitions` reads `subject.transitions`, which `assemble_project` builds from `transition_out` alone — so a stored `transition_in` on a boundary with no Overlap composes nothing and exports untreated. Measured through the shipped routes at `3322ace`. Recorded in `deferred-work.md` with what it would take to close; the shipped toast promises only the half that happens. **Decided 2026-08-31 by R-45:** a boundary with no Overlap is owned by its **outgoing** Shot, so `transition_in` composes a treatment on a Shot's opening frames in exactly one place -- the **first Shot of the plan**, where nothing owns the cut. Composing both halves was rejected because AD-30's mirror makes that one gesture's result, and a Dissolve faded out then in is the picture `Fade through black` is named for. **Built 2026-08-31 by slice 11.f8:** `app._compose_opening_transition` treats the opening frames of the entry the plan lays first, and only where that entry's Shot is also the first in song order -- the two definitions part on a buried head, and R-45's *"where there is no predecessor"* excludes the Shot that buried it. `effects.OPENING_FORMS` is the four mirrors. So the strike-through above is now the amendment's own history rather than the state: both halves of FX-18 ship, each at the one boundary that owns it.)*
...
- A reactive binding compiles to a generated `sendcmd` script, passed as a **cwd-relative filename** — ~~an absolute Windows path's drive colon breaks filter parsing~~ *(cause corrected 2026-08-28 by audit, per R-30: the drive-letter colon does **not** break the parser -- `sendcmd=f=C:/dir/x.cmds`, forward slashes, plain and unquoted, renders and gives a framemd5 identical to the cwd-relative render. A **comma, `=` or `&` anywhere in the path** is what fails, because the filtergraph splits on it, and `lut3d`'s single-quoted colon-escaped form survives those for `sendcmd` too. The cwd-relative remedy still ships and is right; only the reason given for it was false -- and a false reason is what the next reader generalises from.)* and names the wrong filter (AD-22)
...
**FRs covered:** FX-16, FX-17, **FX-18** *(in part when this epic closed; whole since 2026-08-31)*, FX-19, FX-21, FX-NFR-1, FX-NFR-2 *(corrected 2026-08-30: FX-18 names a Shot's opening frames as well as its final ones and only the final ones shipped. Listing it flatly as covered is the third of three places that said so, and the three together are why the gap survived a slice, a review pass and a closing audit — each one read as confirmation of the others. **Closed 2026-08-31 by slice 11.f8**, which built the opening half in the shape R-45 decided; the correction stands as the record of how the gap survived.)*
...
**And** ~~no parameter is specially privileged and none is excluded by category (FX-12)~~ *(amended 2026-08-27, R-25)* **Not literally true and never could have been:** drivability is a property of the (parameter -> filter option) pair. Measured on ffmpeg 7.0, `noise`, `vignette`, `unsharp`, `shufflepixels` and `edgedetect` expose no runtime-settable option at all, and a `sendcmd` aimed at one is silently ignored at rc 0. The genuinely non-drivable parameters are `grain.strength`, `grain.seed`, `vignette.angle`, `sharpen.amount`, `edge_treatment.low`, `edge_treatment.high`, `pixel_shuffle.block`, `pixel_shuffle.seed` and `lut_look.lut` -- the last because a `.cube`'s `file` has no timeline flag. `edge_treatment.strength` and `pixel_shuffle.amount` **are** drivable, because both effects compose as a branch and their dial is written into `blend`'s `all_opacity`. ~~What survives of this clause, and is the half worth keeping: **no parameter is excluded by *family or category*** -- Geometry, Texture, Grade and Stylize are all bindable, and exclusion is per-parameter, measured, and refused by name.~~ **That surviving half is false too, and R-29 said it had amended this criterion when it had not -- corrected 2026-08-28 by audit.** Geometry *is* excluded as a family: every declared number in it is refused (`punch_in.zoom`, `slow_zoom.zoom`, `handheld_shake.amplitude`, `handheld_shake.frequency`, `dutch_tilt.angle`), because a `sendcmd` moving both `crop` dimensions aborts ffmpeg at rc 3 with a truncated file. **And four more exclusions are recorded in no ruling, no criterion and no PRD consequence**: `handheld_shake.frequency`, `bloom.threshold`, `posterize.levels` and `scanlines.lines` all carry `DRIVE_EXPRESSION` -- the parameter reaches its filter inside an expression rather than as a value of its own. **Counted: 18 of 38 declared parameters are drivable.** What actually survives is much smaller and is the only part worth carrying forward: *exclusion is per-parameter, measured against real ffmpeg, and refused by name in the catalogue's own words.* The catalogue is the enumeration; no artifact is.
...
**And** the file is passed to ffmpeg as a **bare relative filename with the process cwd set to its directory** — ~~an absolute Windows path's drive-letter colon parses as a filter option separator and fails naming a filter that is not the problem~~ *(cause corrected 2026-08-28 by audit, per R-30: the drive-letter colon does **not** break the parser -- `sendcmd=f=C:/dir/x.cmds`, forward slashes, plain and unquoted, renders and gives a framemd5 identical to the cwd-relative render. A **comma, `=` or `&` anywhere in the path** is what fails, because the filtergraph splits on it, and `lut3d`'s single-quoted colon-escaped form survives those for `sendcmd` too. The cwd-relative remedy still ships and is right; only the reason given for it was false -- and a false reason is what the next reader generalises from.)* (AD-22)
...
**And** the envelope is drawn in `--blue`, ~~the Trigger Floor as a `--dim` hairline~~, and the existing `--acid` playhead through it, so envelope and picture read against one time axis *(amended 2026-08-27, R-31: the floor is compared against the **band level** while the readout draws the **compiled parameter value** — different units, so a hairline at the floor's number names a value it has nothing to say about. The `--dim` hairline is the **rest line**, and the floor is drawn as ground under the silenced runs. Below the floor a `punch` drive is exactly zero, so colour alone could not have marked it: the silenced line lies on the rest line in the same token, and the state needs width.)*
...
**Then** `--blue: #5b9bd5` is added as the ~~sixth~~ **seventh** and final accent *(count corrected 2026-08-28: `styles.css` defines `--acid`, `--acid-ink`, `--amber`, `--red`, `--red-edge` and `--cyan` before it; the artifact and the stylesheet cannot both be right about how many there are)*, reserved to transitions and reactive bindings, and DESIGN.md's "no new accent colors" anti-goal is amended in place with the argument for the exception (UX-DR1).
